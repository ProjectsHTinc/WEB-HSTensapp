...
for($i = 0; $i < $dataSize; $i++) 
	{
        ...
        $query="UPDATE user_purchase_history SET status='Success',purchase_order_id = '$orderid' WHERE id = '$purchase_id'";
        $objRs  = mysql_query($query) or die("Could not select Query ");

    	if($order_status=="Success")
    	{
            $enc_purchase_id = base64_encode($purchase_id);
            header("Location: https://ensyfi.com/testing/user/order_confirm/".$enc_purchase_id."");
    	}

    	if($order_status=="Aborted")
    	{
    	   header("Location: https://ensyfi.com/testing/");
    	}
